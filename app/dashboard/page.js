...
const formatMessageContent = (content) => {
  ...
  const formattedLines = lines.map((line, index) => {
    ...
    } else if (formattedLine.match(/^\d+\.\s/)) {
      // Keep numbered lists as is
    }
    
    // Format headers (lines that look like headers)
    else if (formattedLine.match(/^[A-Z][^.!?]*:$/)) {
      formattedLine = `**${formattedLine}**`;
    }
    
    // Format code-like content (text in backticks or that looks like code)
    else if (formattedLine.includes('`') || formattedLine.match(/^[A-Z_]{2,}$/)) {
      ...
      {paragraphs.map((paragraph, index) => {
        ...
        const formattedLines = lines.map((line, lineIndex) => {
          ...
          if (trimmedLine.startsWith('• ')) {
            return (
              <Box key={lineIndex} sx={{ display: 'flex', alignItems: 'flex-start', mb: 0.5 }}>
                <Typography component="span" sx={{ mr: 1, color: '#00BFA5', fontSize: '1.2em' }}>•</Typography>
                <Typography component="span" variant="body2">{trimmedLine.substring(2)}</Typography>
              </Box>
            );
          }
          ...
          if (trimmedLine.match(/^\d+\.\s/)) {
            ...
            const text = trimmedLine.substring(number.length + 2);
            return (
              <Box key={lineIndex} sx={{ display: 'flex', alignItems: 'flex-start', mb: 0.5 }}>
                <Typography component="span" sx={{ mr: 1, color: '#00BFA5', fontWeight: 'bold', minWidth: '20px' }}>
                  {number}.
                </Typography>
          ...
          if (trimmedLine.endsWith(':') && trimmedLine.length > 3) {
            return (
              <Typography key={lineIndex} variant="body1" sx={{ 
                fontWeight: 'bold', 
                color: '#00BFA5', 
                mb: 1, 
                mt: index > 0 ? 2 : 0 
          ...
          // Regular text
          return (
            <Typography key={lineIndex} variant="body2" sx={{ mb: 0.5, lineHeight: 1.6 }}>
              {trimmedLine}
            </Typography>
          );
        });
